...
class OmniBox extends Component {
  ...
  onKeyPress(event){
    if (event.nativeEvent.key == 'Enter' && this.state.newValue) {
      var newDataItem = new BookModel(this.state.newValue);

      var dataList = this.props.data;
      var dataItem = Utils.findBook(newDataItem, dataList);
      if(dataItem) {
  ...
  render() {
    return (
      <TextInput style={{height: 36, padding: 4, marginBottom: 0, fontSize: 16, borderWidth: 1, borderColor: '#eee', borderRadius: 8, backgroundColor: '#fff'}}
        placeholder='Add a book or Search'
        blurOnSubmit={false}
